...
void moveForward() { // Avanzar hacia adelante
  myServo.write(81);  // Vuelve el servomotor a la posición inicial
...
void turnLeft() { // Giro hacia la izquierda
  myServo.write(58); // (Regular el valor de giro)
  digitalWrite(IN1, HIGH);
  digitalWrite(IN2, LOW); 
  analogWrite(ENA, velocidadGiro );  // Controla la velocidad del motor con el valor fijo establecido
}
...
void loop() {
  ...
  if (distance < 15) { // si la distancia delantera es menor a 35cm 
    stop();
    delay(1000); // Espera 1 segundos
    moveBack();
    delay(1300); // Retrocede por 1.3 segundos
    stop(); // Detiene después de retroceder
    delay(500);
  } else if (distance <= 50 ) {  // Si solo el sensor delantero detecta un obstáculo a 45 cm o menos
    turnLeft();
