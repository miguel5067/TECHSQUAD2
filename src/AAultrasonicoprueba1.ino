...
void moveBack() { // Retroceder
  myServo.write(125); // Vuelve el servomotor a la posición inicial
...
void loop() {
  ...
  if (distance < 40) { // si la distancia delantera es menor a 35cm 
    stop();
    delay(1000); // Espera 1 segundos
    moveBack();
    delay(1750); // Retrocede por 1.7 segundos
    stop(); // Detiene después de retroceder
  } else if (distance <= 60) {  // Si solo el sensor delantero detecta un obstáculo a 45 cm o menos
    turnLeft();
